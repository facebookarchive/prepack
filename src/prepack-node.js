...
export function prepackString(
  ...
  if (options.serialize || !options.residual) {
    ...
    let serialized = serializer.init(options.filename || filename, code, sourceMap, options.sourceMaps);
    if (!serialized) {
      throw new InitializationError();
    }
    if (!options.residual) return serialized;
    let result = realm.$GlobalEnv.executePartialEvaluator(filename, serialized.code, JSON.stringify(serialized.map));
