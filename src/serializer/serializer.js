...
import { BoundFunctionValue, ProxyValue, SymbolValue, AbstractValue, EmptyValue, FunctionValue, Value, ObjectValue, PrimitiveValue, NativeFunctionValue, UndefinedValue, NumberValue } from "../values/index.js";
...
export class Serializer {
  ...
  _serializeValueDataView(name: string, val: ObjectValue, reasons: Array<string>): BabelNodeExpression {
    let elems =[];
  }
  ...
  _serializeValueObject(name: string, val: ObjectValue, reasons: Array<string>): BabelNodeExpression {
    ...
    switch (kind) {
      ...
      case "DataView":
        this.logger.logError(val, `Serialization of DataView is not supported`);
        return this._serializeValueDataView(name, val, reasons);
      case "Map":
      case "WeakMap":
