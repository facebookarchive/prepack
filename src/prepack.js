...
function run_internal(
  ...
  let serialized = new Serializer({ partial: true, compatibility, mathRandomSeed, debugNames, singlePass }, { initializeMoreModules: speculate, internalDebug: true, trace }).init(name, raw, map, outputMap !== undefined);
  if (!serialized) {
    process.exit(1);
    invariant(false);
  }
