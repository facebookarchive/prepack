...
let reactNativeCode = `
  function createReactNative(React, Platform, NativeModules) {
    ...
    ReactNativeViewAttributes.UIView = {
      ...
      style: null // ReactNativeStyleAttributes,
    };
    ...
    return {
      StyleSheet,
      Text,
      View: requireNativeComponent("RCTView"),
    };
  }
`;
...
export function createMockReactNative(realm: Realm, reactNativeRequireName: string): ObjectValue {
  ...
  let Platform = AbstractValue.createAbstractObject(realm, `require("Platform")`);
  let NativeModules = AbstractValue.createAbstractObject(realm, `require("NativeModules")`);
  let UIManager = AbstractValue.createAbstractObject(realm, `require("NativeModules").UIManager`);
  let template = new ObjectValue(realm, realm.intrinsics.ObjectPrototype);
  Properties.Set(realm, template, "UIManager", UIManager, true);
  template.makePartial();
  template.makeSimple();
  NativeModules.values = new ValuesDomain(new Set([template]));

  let reactNativeValue = factory(realm.intrinsics.undefined, [reactLibrary, Platform, NativeModules]);
